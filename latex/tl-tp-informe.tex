...
% OJO: Aclarar que si bien es una gramática de atributos, por claridad
%      no vamos a escribir acá cuáles van a ser esos atributos. Eso va
%      a quedar explicitado cuando hablemos de semántica y de implementación.
% Piden decir de qué tipo es la gramática, no nos olvidemos de eso.
% También hay que documentar bien qué cosas *no* se pueden hacer con esta
% gramática (lambda/ifthenelse a la derecha de una aplicación) y por qué
% decidimos que quede así.
...
% TODO: Ver bien cómo ubicar estas figuras. Por ahí directamente hacer que
%       dejen de ser figuras.
\begin{tabular}{lrrl}
$\mathcal{P}:$
& $S$  & \produces     & $E$ \\
